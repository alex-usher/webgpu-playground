...
const saveShaderCode = (
    ...
    if (isPublic) {
        addDoc(collection(firedb, "public-shaders"), shaderDoc).then(() => {
    ...
    if (user) {
        ...
        addDoc(usersShadersRef, shaderDoc).then(() => {
            enqueueSnackbar('Successfully saved!', {variant: 'success', autoHideDuration: 1000})
        }).catch(err => {
            enqueueSnackbar('Failed to save', {variant: 'error', autoHideDuration: 1000})
        })
...
const FormDialog = ({open, handleClose, vertexCode, fragmentCode}: FormDialogProps) => {
    ...
    return (
        <Dialog open={open} onClose={resetAndClose}>
            ...
            <DialogContent>
                ...
                <TextField autoFocus margin="dense" id="name" label="Name of shader file" type="text" fullWidth
                           variant="standard" onChange={(name) => setFileName(name.target.value)}
                />
                <FormGroup>
                    <FormControlLabel
                        onChange={(e) =>
            ...
            <DialogActions>
                ...
                <Button
                    onClick={() => {
                        saveShaderCode(vertexCode, fragmentCode, fileName, isPublic, enqueueSnackbar)
                        resetAndClose()
                    }}>
                    Save
                </Button>
